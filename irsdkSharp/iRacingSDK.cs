...
namespace iRSDKSharp
{
    ...
    public class Defines
    {
        ...
        public string MemMapFileName = "Local\\IRSDKMemMapFileName";
    ...
    public class iRacingSDK
    {
        ...
        public int VarSubHeaderSize = 32;

        public bool IsInitialized = false;

        MemoryMappedFile iRacingFile;
       public MemoryMappedViewAccessor FileMapView;


        public CiRSDKHeader Header = null;
        public CiRSDKSubHeader SubHeader = null;
        ...
        public bool Startup()
        {
            try
            {
                iRacingFile = MemoryMappedFile.CreateFromFile(defines.MemMapFileName);
                FileMapView = iRacingFile.CreateViewAccessor();

                //build file header
                Header = new CiRSDKHeader(FileMapView);

                //build file subheader
                SubHeader = new CiRSDKSubHeader(FileMapView);

                //get telemetry headers
            ...
            if (IsInitialized && Header != null)
            {
                if (VarHeaders.ContainsKey(name))
                {
                    ...
                    else if (VarHeaders[name].Type == CVarHeader.VarType.irFloat)
                    {
                        ...
                        {
                            return FileMapView.ReadSingle(Header.Buffer + varOffset);//556 is buffer length
                        }
                    }
    ...
    //32 bytes
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
    public struct VarSubHeader
    {
        public DateTime /*time_t*/ sessionStartDate;
